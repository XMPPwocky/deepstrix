...
- (i) **Localhost-only status endpoint** (requested 2026-09-24). Today the only way to see what is in flight is to reconstruct it from the log; `/readyz` just says `inflight=true`.
  ...
  - **Localhost only, enforced structurally.** Serve it from a separate listener bound to `127.0.0.1` (e.g. `:18081`), not from the main router. The main router also listens on the tailnet address, so a peer-address check on that router is one refactor away from exposing it.
  ...
  - **Gates:**
    ...
    - `curl <tailnet-ip>:18081/v1/status` must fail (the listener isn't there)
...
- **Low-bit expert kernels: KEEP (owner, 2026-09-25).** IQ2_XXS/IQ2_S/IQ2_XS, IQ3_XXS/IQ3_S, Q2_K matvec and `v4flash-core/src/iq3_s_ref.rs` stay. They are the candidate formats for an imatrix-calibrated requant of all 15,360 V4.1 experts: IQ2_S fits them in ~175 GB, which would end expert paging. Collect the imatrix from live traffic, prefill and decode both. Under CED, decoder layers 20–39 run their MoE on only ~21% of prefill rows, so decode supplies most of their samples, and an offline prefill-only run would under-sample them ~5×. Production supplies ~4.7M rows/day to encoder layers and ~1.8M to decoder layers. One day leaves 300 of 15,360 experts under 256 samples, carrying 0.01% of routed picks. Shrink those toward the layer mean; 3 experts were never picked in 8.5M tokens.
  - Their oracle tests run against V4 dumps. Re-fixture them from the V4.1 golden corpus; don't delete them.
  - **A requant must balance MXFP4's exact zeros first.** ~11.7% of the experts' weights are exactly 0, IQ2 grids have no zero, and ggml's quantizer signs +0.0 positive. Every zero then becomes the same small positive value, a coherent per-column bias. In the CPU reference this made an all-experts IQ2_S model collapse (KL 15 nats) and inflated IQ2_XXS's cost. Give the zeros balanced random signs before quantizing (`scripts/v41_oracle/expert_rt.py::balance_zero_signs`, `68f823c`).
