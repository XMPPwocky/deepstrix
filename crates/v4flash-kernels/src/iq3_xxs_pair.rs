...
impl Iq3XxsPairMatvec {
    ...
    pub fn launch_fused_swiglu_kwide(
        ...
        if n_rows % 8 != 0 {
            return Err(eyre!("iq3_pair kwide: n_rows={n_rows} not %8"));
        }
        if n_blocks % 2 != 0 {
            return Err(eyre!("iq3_pair kwide: n_blocks={n_blocks} must be even"));
        }
        if chunk_size > 32 {
            return Err(eyre!(
                "iq3_pair kwide: chunk_size={chunk_size} exceeds IQ3_KW_MAX_CHUNK=32"
            ));
        }
        if n_work_items == 0 {
            return Ok(());
        ...
        let function = self
            .module
            .get_function("iq3_xxs_pair_matvec_fused_swiglu_kwide")?;
        let cfg = LaunchConfig {
            grid: (n_rows / 8, n_work_items, 1),
            block: (256, 1, 1),
            shared_mem_bytes: 0,
