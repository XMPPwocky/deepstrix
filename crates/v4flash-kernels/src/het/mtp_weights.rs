...
//!
//! Loaded from a SEPARATE GGUF file (antirez ships
...
//!   no long-context compression needed).
...
use crate::forward::{
    load_f32_weight, HC_DIM, HC_MIX_DIM, N_EMBD, N_HC, N_HEAD, N_HEAD_DIM, N_LORA_Q,
};
...
pub struct MtpWeights {
    ...
    // ---- MTP-specific head (dGPU). Reuses the main model's output_norm
    // / token_embd / output (vocab projection) — only hc_head_* differ. ----
...
impl MtpWeights {
    ...
    /// (separate file from the main model). `rope_params_for_layer(N_LAYER)`
    /// supplies the MTP layer's RoPE params — usually identical to the
    /// last main-model layer's, since MTP shares the same context.
    pub fn load(
        mtp_gguf: &MappedGguf,
        dgpu_device: Device,
        igpu_device: Device,
...
fn load_f32_as_f16_device_weight(
    ...
    // Convert each f32 → IEEE f16 via the table-less builtin from
    // iq2_xxs_tables (round-to-nearest-even, handles subnormals).
...
fn f32_to_f16_bits(f: f32) -> u16 {
    ...
    if exp32 == 0xff {
        // Infinity / NaN
        let m = if frac != 0 { 0x200 } else { 0 }; // preserve NaN-ness, payload truncated
    ...
    if exp16 <= 0 {
        if exp16 < -10 {
            return sign << 15; // underflow to ±0
        }
        // Subnormal: shift down the implicit-leading-1 mantissa.
        let frac_with_hidden = (frac | 0x800000) as u32;
        let shift = (14 - exp16) as u32; // shift amount in [4, 24]
