...
//!   is B-extended to a single contiguous `[rows × per_token_size]` buffer.
...
//! at `B_MAX.div_ceil(2)`. The single-lane driver (`forward_prefill`)
//! needs `rows >= B_MAX`.
//!
//! Memory per lane at `rows = 512` (B_MAX = 1024, two lanes): dGPU
//! ~0.74 GiB, iGPU ~98 MiB. See the per-field comments for the
//! disjoint-lifetime unions (R1/R2/R3 arenas) that get it there.
...
/// chunk is split across two pipeline lanes, so per-lane scratch is
/// sized at `B_MAX.div_ceil(2)` rows (see [`BatchDgpuScratch::alloc_rows`]).
/// The KV cache is oversized by `B_MAX` rows (`state::KV_CACHE_ROWS`)
/// because both lanes append into the same chunk.
...
///
/// ## Lifetime unions (all on the single `de.compute` stream, program
/// ordered within one lane's `forward_layer_pre_moe_v2` + `_post_moe_v2`)
///
/// Buffers whose live ranges never overlap within a lane share one
/// allocation. Lanes never share anything — lane A and lane B each own a
/// full `BatchDgpuScratch`. Phase order per layer (all de.compute):
/// P1 mhc_pre_attn → P2 q-chain → P3 kv-chain → P4 kv-append+compressor →
/// P5i indexer → P5a score/smwsum → P5e SWA evict → P6 out-proj →
/// P7 mhc_post_attn → P8 mhc_pre_ffn → P9 router → P10 shared-expert →
/// P11x peer push → P11h hot-expert leg → P11i iGPU chain → (lane switch)
...
///   P11h (the hot reduce writes the owned `ffn_moe_dgpu`, which is what
///   P12 reads after the lane switch).
...
pub struct BatchDgpuScratch {
    ...
    /// `[B, Q_FLAT]` — normed + roped Q. Owned; also the R2 backing
    /// allocation for the P6/P7 out-proj temporaries (see struct doc).
    ...
    /// for the batched split attention kernels (long-context prefill).
    ...
    /// `[B, ceil(ATTN_MIXED_MAX_KEYS/32)]` — bitpacked CSA mask written
    /// by the batched topk kernel (self-zeroed + atomicOr per token). Bit
    /// `(c & 31)` of word `b * max_keys_words + (c >> 5)` is 1 iff comp
    /// row c is allowed for batch token b. The production score kernel
    /// receives `None` for the mask (the CSA gather handles sparsity), so
    /// this is currently write-only scratch; it stays because the topk
    /// kernel writes it unconditionally. 3 MiB at rows=512.
    ...
    /// indices per token, sentinel -1 in unused slots). 1 MiB at rows=512.
    ...
    /// per-row mask tests on the full sparse n_comp. 256 MiB at rows=512.
    ...
    /// main: `rows/4` boundaries × 8 × 1024 f32 = 4 MiB at rows=512).
    ...
    /// `Some` only when `DGPU_HOT_EXPERTS > 0` (~81 MiB of R1 views + 8 MiB
    /// owned `ffn_moe_dgpu` + member/work-item lists at rows=512).
...
/// goes to the OWNED `ffn_moe_dgpu`, so nothing in R1 is read after P11h.
/// The next layer's P1 `flat` write is the next R1 access. Earlier R1
/// users (heads/flat) leave stale f32 bit patterns (possibly NaN) in the
/// miss slots of `mid_cat`; the post-gate/up q8k quantize reads them but
...
pub struct BatchDgpuHotScratch {
    ...
    /// ffn_moe_recv at ffn_combine (P12, AFTER the lane switch). OWNED —
    /// it is the one hot buffer that outlives the pre-MoE call.
...
impl BatchDgpuHotScratch {
    ...
    fn alloc(id: i32, rows: usize, r1: &DeviceBuffer<u8>) -> eyre::Result<Self> {
        ...
        Ok(Self {
            ...
            midq_cat,
            partials,
            ffn_moe_dgpu: DeviceBuffer::new(id, rows * N_EMBD as usize)?,
...
/// `rows × per_token_size`. Used by the batched iGPU MoE in
/// `forward_layer_batch_v2`: one peer-push of `[B, N_EMBD]` ain, one
/// batched iq2 + q2k call chain, one peer-push of `[B, N_EMBD]` ffn_moe
/// back. ~98 MiB at rows=512 (q2k_partials 48, d_mid_cat 24, recv/moe 8
/// each, d_midq_cat 6.9, d_xq_q8k 2.3, expert_members 0.5, work lists).
pub struct BatchIgpuScratch {
    ...
    /// `q2_k_reduce_partials`. `[B*n_used, N_EMBD]` f32 = 48 MiB at rows=512.
...
impl BatchIgpuScratch {
    ...
    pub fn alloc_rows(igpu_device: Device, rows: usize) -> eyre::Result<Self> {
        eyre::ensure!(rows > 0 && rows <= B_MAX, "BatchIgpuScratch rows={rows} out of (0, B_MAX]");
...
/// view pack (see [`BatchDgpuScratch`] doc).
...
impl BatchDgpuScratch {
    ...
    /// lane at that size (~0.74 GiB dGPU vs ~1.49 GiB at B_MAX). Every
    /// batched entry point checks `b <= rows` before touching the scratch.
    pub fn alloc_rows(dgpu_device: Device, rows: usize) -> eyre::Result<Self> {
        eyre::ensure!(rows > 0 && rows <= B_MAX, "BatchDgpuScratch rows={rows} out of (0, B_MAX]");
        dgpu_device.set_current()?;
        ...
        // positions of this lane's `rows` tokens.
        ...
        Ok(Self {
            ...
            // 512 × 1544 words = 3 MiB at rows=512. Write-only today (see
            // field doc) but the topk kernel needs a real target.
