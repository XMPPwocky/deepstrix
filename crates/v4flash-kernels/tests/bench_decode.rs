...
/// M40-P1.7: bench `forward_pair` (layer-major K=1 verify-pair forward).
/// Compare to the single-token bench above: pair should run at ~1.1-1.3×
/// single-token wall, validating that iGPU MoE for token0 hides behind
/// token1's dGPU pre-MoE work.
///
/// Usage: same env vars (BENCH_TOKENS, BENCH_WARMUP) as bench_decode_het_parallel.
/// Reports ms per pair (2 tokens of decode work).
#[test]
#[ignore]
fn bench_decode_pair_het_parallel() -> eyre::Result<()> {
    install_panic_handler()?;

    let n_pairs: i32 = std::env::var("BENCH_TOKENS")
        .ok()
        .and_then(|s| s.parse().ok())
        .map(|t: i32| t / 2)
        .unwrap_or(32);
    let warmup_tokens: i32 = std::env::var("BENCH_WARMUP")
        ...
        .and_then(|s| s.parse().ok())
        .unwrap_or(8);
    eprintln!("pair bench config: n_pairs={n_pairs} (= 2× {n_pairs} = {} tokens of work), warmup_tokens={warmup_tokens}", n_pairs * 2);
    ...
    let mut dgpu_scratch = DgpuScratch::alloc(dgpu)?;
    let mut igpu_scratch = IgpuScratch::alloc(igpu)?;
    // pair_tokens are at positions [warmup_tokens, warmup_tokens + 2*n_pairs).
    // Need room in state for all of these.
    let total_positions =
        warmup_tokens + 2 * n_pairs + PROMPT_TOKENS.len() as i32 - 1;
    let mut state = HetModelState::alloc(dgpu, igpu, total_positions as u32)?;

    let max_inp_pos = dump.n_logit_rows as i32 + PROMPT_TOKENS.len() as i32 - 2;
    eprintln!("preloading {} input residuals...", total_positions);
    ...
    // Warmup: fill state with `warmup_tokens` single-token forwards.
    eprintln!("warmup: {} single-token forwards...", warmup_tokens);
    ...
    // Pair bench loop.
    eprintln!("pair bench: {} pairs (positions {}..{})", n_pairs, warmup_tokens, warmup_tokens + 2 * n_pairs);
    let mut pair_us: Vec<u64> = Vec::with_capacity(n_pairs as usize);
    let bench_start = Instant::now();
    for k in 0..n_pairs {
        let pos = warmup_tokens + 2 * k;
    ...
    let mut sorted = pair_us.clone();
    sorted.sort_unstable();
    let pair_count = pair_us.len() as f64;
    let pair_total: u64 = pair_us.iter().sum();
    let median_us = sorted.get(sorted.len() / 2).copied().unwrap_or(0);
    let min_us = *sorted.first().unwrap_or(&0);
    let max_us = *sorted.last().unwrap_or(&0);
    let avg_us = pair_total as f64 / pair_count;
    // Effective tok/s assuming both tokens accepted: 2 tokens per pair.
    let effective_tps_full_accept = 2_000_000.0 / avg_us;
    // Effective tok/s at 0% acceptance: 1 token per pair (the target's prediction
    // after token0; the drafted token1 always wrong is the worst case).
    let effective_tps_no_accept = 1_000_000.0 / avg_us;

    eprintln!(
        "BENCH PAIR (n={} pairs after {} token warmup): wall={:.2}s avg={:.2}ms median={:.2}ms min={:.2}ms max={:.2}ms",
        pair_us.len(),
        warmup_tokens,
        bench_wall.as_secs_f64(),
        avg_us / 1000.0,
    ...
    eprintln!(
        "  effective tok/s: {:.2} (full accept) | {:.2} (no accept)",
        effective_tps_full_accept, effective_tps_no_accept,
    );
    eprintln!("BENCH PAIR percentiles (ms):");
    let pcts: [(f64, &str); 6] = [
        (10.0, "p10"),
        (50.0, "p50"),
        (75.0, "p75"),
    ...
    for (p, label) in pcts {
        ...
        let us = sorted[idx];
        eprintln!(
            "  {label} {:>7.2} ms  ({:>5.2} full-accept tok/s)",
            us as f64 / 1000.0,
            2_000_000.0 / us as f64,
        );
    }
    Ok(())
}

/// M40-P3.4: bench `forward_pair_interleaved` (substage-interleaved K=1 verify pair).
/// Compare to bench_decode_pair_het_parallel above. Interleaved should be faster
/// because t1's dGPU pre_moe overlaps with t0's iGPU MoE.
#[test]
#[ignore]
fn bench_decode_pair_interleaved_het_parallel() -> eyre::Result<()> {
    install_panic_handler()?;
    ...
    let warmup_tokens: i32 = std::env::var("BENCH_WARMUP")
        ...
        .unwrap_or(8);
    eprintln!(
        "interleaved pair bench config: n_pairs={n_pairs} (= {} tokens of work), warmup_tokens={warmup_tokens}",
        n_pairs * 2
    );
    ...
    let mut dgpu_scratch = DgpuScratch::alloc(dgpu)?;
    let mut igpu_scratch = IgpuScratch::alloc(igpu)?;
    let total_positions = warmup_tokens + 2 * n_pairs + PROMPT_TOKENS.len() as i32 - 1;
    let mut state = HetModelState::alloc(dgpu, igpu, total_positions as u32)?;
    ...
    eprintln!(
        "interleaved pair bench: {} pairs (positions {}..{})",
        n_pairs,
        warmup_tokens,
        warmup_tokens + 2 * n_pairs
    );
    let mut pair_us: Vec<u64> = Vec::with_capacity(n_pairs as usize);
    let bench_start = Instant::now();
    for k in 0..n_pairs {
    ...
    eprintln!(
        "BENCH INTERLEAVED PAIR (n={} pairs after {} token warmup): wall={:.2}s avg={:.2}ms median={:.2}ms min={:.2}ms max={:.2}ms",
        pair_us.len(),
        warmup_tokens,
        bench_wall.as_secs_f64(),
        avg_us / 1000.0,
    ...
    eprintln!("BENCH INTERLEAVED PAIR percentiles (ms):");
    let pcts: [(f64, &str); 6] = [
        (10.0, "p10"),
        (50.0, "p50"),
        (75.0, "p75"),
