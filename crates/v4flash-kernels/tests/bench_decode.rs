...
fn bench_decode_het_parallel() -> eyre::Result<()> {
    ...
    // COMP_KV_FP8_SWEEP=1: run the whole bench twice after ONE weight load,
    // packed-FP8 compressed-KV store then f16 store (COMP_KV_FP8=0), each
    ...
    if sweep { eprintln!("=== end variant {vname} ==="); }
    }
    Ok(())
}
