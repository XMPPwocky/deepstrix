//! GPU oracle for the IQ3_XXS gate/up fused-SwiGLU PAIR kernels, versus the
...
//!
//! The CPU side (`cpu_dot_iq3_xxs_q8_k`) is itself pinned against
//! llama.cpp's own reference in the iq3_xxs down-path oracle, so a pass here
...
use v4flash_kernels::iq3_xxs_pair::{Iq3XxsPairMatvec, BLOCK_IQ3_XXS_PAIR_BYTES};
...
#[test]
#[ignore]
fn iq3_xxs_pair_kernels_match_cpu() -> eyre::Result<()> {
    install_panic_handler()?;
    let igpu = pick_igpu()?;
    igpu.set_current()?;
    let arch = igpu.properties()?.gcn_arch_name;
    let stream = Stream::new(igpu.id)?;
    let iq2s = Iq3XxsPairMatvec::for_arch(&arch)?;
    ...
    let bpe = n_rows * nb * BLOCK_IQ3_XXS_PAIR_BYTES;
    ...
    for w in [&mut gate_h, &mut up_h] {
        for &e in &used {
            for r in 0..n_rows {
                for bi in 0..nb {
                    let o = e * bpe + (r * nb + bi) * BLOCK_IQ3_XXS_PAIR_BYTES;
                    let d = F16_SCALES[(rng.next() & 3) as usize].to_le_bytes();
                    w[o..o + 2].copy_from_slice(&d);
                    for i in 2..BLOCK_IQ3_XXS_PAIR_BYTES {
                        w[o + i] = rng.next_byte();
                    }
                }
            }
    ...
    for (s, &e) in sel.iter().enumerate() {
        let xq_s = &xq_host[..];
        for row in 0..n_rows {
            let go = (e as usize) * bpe + row * nb * BLOCK_IQ3_XXS_PAIR_BYTES;
            let g = cpu_dot_iq3_xxs_q8_k(nb, &gate_h[go..go + nb * BLOCK_IQ3_XXS_PAIR_BYTES], xq_s);
            let u = cpu_dot_iq3_xxs_q8_k(nb, &up_h[go..go + nb * BLOCK_IQ3_XXS_PAIR_BYTES], xq_s);
    ...
    check("iq3_xxs_pair fused batch", &got, &want, 1e-2)?;
    ...
    check("iq3_xxs_pair hetsplit m0+m1", &sum, &want, 1e-2)?;
    ...
    for &(e, bi, sl) in &touched {
        ...
        for row in 0..n_rows {
            let go = e * bpe + row * nb * BLOCK_IQ3_XXS_PAIR_BYTES;
            let g = cpu_dot_iq3_xxs_q8_k(nb, &gate_h[go..go + nb * BLOCK_IQ3_XXS_PAIR_BYTES], xq_s);
            let u = cpu_dot_iq3_xxs_q8_k(nb, &up_h[go..go + nb * BLOCK_IQ3_XXS_PAIR_BYTES], xq_s);
