    ...
    fn decode_step(&mut self, state: &mut WorkerState) -> eyre::Result<()> {
        ...
        if profile {
            ...
            if v4flash_kernels::het::b2_mirror::wanted() {
                let (p, av, sw, bl, fl) = v4flash_kernels::het::b2_mirror::take_sub_stats();
                for (name, v) in [("sub.predicted_miss", p as f64), ("sub.reads_avoided", av as f64), ("sub.picks_swapped", sw as f64), ("sub.blocked", bl as f64), ("sub.plan_failed", fl as f64)] {
                    let e = acc.stages.entry(("host", name)).or_insert((0.0, 0));
                    e.0 += v; e.1 += 1;
                }
            }
